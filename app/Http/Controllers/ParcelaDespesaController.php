...
class ParcelaDespesaController extends Controller
{
    ...
    {
        $reparcelaDespesaRepository = new ParcelaDespesaRepository();
        $reparcelas = $reparcelaDespesaRepository->getReparcelasByDespesa($idDespesa);

        $parcelaDespesaRepository = new ParcelaDespesaRepository();
        $parcelas = $parcelaDespesaRepository->getParcelasByDespesa($idDespesa);
        
        
        if(!empty($reparcelas[0])){
            return response()->json($reparcelas);
        }
        else{
    ...
    public function getReparcela($idParcela)
    {
        $despesaRepository = new DespesaRepository();
        $parcelaDespesaRepository = new ParcelaDespesaRepository();
        $rateioRepository = new RateioRepository();
        $parcela = $parcelaDespesaRepository->getReparcela($idParcela);
        $rateios = $rateioRepository->findRateioDespesa($parcela->fk_despesa);
    ...
    public function setProvisionDateREPARCELA(Request $request)
    {
        try {
            $provisionDate = $request->date;
            $ExpenseIds = $request->ids;
            ...
            foreach ($ExpenseIds as $id) {
                ParcelaDespesa::setProvisionDateREPARCELA($id, $provisionDate);
            }

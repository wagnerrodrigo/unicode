...
function geraParcelas(valorTotal, numeroParcelas) {
    ...
    if (numeroParcelas.value > 0 && valorFinal > 0) {
        removeItens();
        var valorParcela = valorFinal / numeroParcelas.value;
        var arredondado = arredondaNumero(Number(valorParcela), 2);
...
function arredondaNumero(numero, casasDecimais) {
...
function atribuiValorAReparcelaGerada() {
    
    let parcelasGeradas = document.getElementsByClassName("parcela_gerada");
    var parcelasGeradasHidden = document.getElementsByClassName("parcela");
    ...
    if (valorTotalParcelas.toFixed(2) != valorTotalDespesa) {
        swal({
        ...
        document.getElementById("form_reparcela").submit();
    }
}
...
function geraDataVencimentoParcelas(parcela) {
    ...
    return new Date(date.setMonth(date.getMonth() + parcela))
        .toISOString()
        .split("T")[0];
